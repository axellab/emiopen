<%@ Page Language="VB" AutoEventWireup="false" CodeFile="fixture2.aspx.vb" Inherits="fixture_fixture2" %>
...
<div class="row" style="padding-left:20px; padding-top: 20px">
    ...
    <div class="col-md-5 col-sm-12" hidden>   <br><br>  
        <FORM>
            <div class="row">
    ...
    <table class="table table-dark" style="padding-left:10px;">
      ...
      <div class="col-md-6 col-sm-12">   <br><br>  
        <FORM>
            <div class="row">
                  <div class="col-10 flex" id="ag2"><select name="" class="form-control" id="selecter"><option>Seleccione un grupo</option></select><button type="button" onclick="agregarjugador()" class="btn btn-success">+</button></div>
  ...
  function () {
      console.log('El DOM  esta listo!');
        ...
        $(document).ready(function () {
            ...
            if (anuncioParam > 0) {
                ...
                xhr.onload = function () {
                    ...
                    console.log(json);
                    for (x of json) {
                        dev = dev + "<tr name='grup'>";
                        dev = dev + "<td style='cursor:pointer'  id='GRUPO23' onclick='traerjugadores(" + x.id + ");'>" + x.nombre + "</td>";
                        dev = dev + "<td style='text-align:right;' ><button type='button'  name='el' onclick='eliminargrupo(" + x.id + ")' class='btn btn-danger'>X</button></td>";
                        dev = dev + "</tr>";
                    }
                    $("#area").html(dev);
                    document.all['GRUPO23'].click();
                };
        ...
        function cargarselect() {
            ...
            $("#selecter").html("");
            var deve = "<option value='0'>Seleccione un jugador</option>";
            xhr.onload = function () {
                var json = JSON.parse(xhr.response);
                console.log(json);
                for (x of json) {
                    deve = deve + "<option value='" + x.id + "'>" + x.nombre + " " + x.apellido + "</option>";
                }
                $("#selecter").html(deve);
        ...
        function verpartidos() {
            if (document.documentElement.clientWidth > 980) {
                window.location = "http://44.204.37.84/ADMIN/fixture/eliminacion.aspx?id=" + anuncioParam
            } else {
                window.location = "http://44.204.37.84/ADMIN/fixture/eliminacioncel.aspx?id=" + anuncioParam
            }
        }

        function generarpartidos() {
            
            if (jugadorescant != 2 && jugadorescant != 4 && jugadorescant != 8 && jugadorescant != 16 && jugadorescant != 32) {
                alert("La cantidad de jugadores no es correcta para iniciar la competencia " + jugadorescant);
                return
            }
